...
def imgtool_draw(img_name, img_name_pre = False):
    ...
    img2 = Imageuploaddraw(image_file=img_content, title= out_f_name.split('.')[-2], date_of_upload = date_of_upload)
    img2.save()
    ...
    if img_name_pre is not False:
        ...
        img3 = Imageuploaddraw(image_file=img_content, title= pre_f_name.split('.')[-2])
        img3.save()
        return (img2.image_file.url, img3.image_file.url)
    return img2.image_file.url

#google image search
def google_image_draw(keyword, num=12, no_download=True):
...
def flickr_image_draw(keyword, num=12, download=False):
