...
def matting_result(pic_input, tri_input, title, model, cuda, website = False):
    if website:
        img = pic_input
    else :
    ...
    img = Imageuploadmask(image_file=img_content, title= title.split('_mask')[0]+'_matting', date_of_upload = date_of_upload)
    img.save()
