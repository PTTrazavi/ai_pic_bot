...
def trimap(image, title, size, erosion=False):
    ...
    img = Imageuploadmask(image_file=img_content, title= title.split('_mask')[0]+'_trimap', date_of_upload = date_of_upload)
    img.save()

    return remake, img.image_file.url #original trimap, saved trimap
